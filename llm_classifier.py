...
EXIT  - fully closing the ENTIRE remaining position right now: "selling all/everything/the rest/what's left", "I'm out", "closed it all", "sold mine", "cutting my puts/calls/position", "cutting here", "I have to cut it", "had to cut it", "cut it" — the whole thing, not a fraction.
...
async def classify(text, client, model, provider="anthropic", recent_messages=None):
    ...
    docstring. client: whatever llm_providers.make_client(provider, ..., is_async=True)
    ...
    try:
        user_content = _build_user_content(text, recent_messages)
        tool_input = await llm_providers.classify_async(
            provider, client, model, SYSTEM_PROMPT, ROUTE_TOOL, user_content,
