...
CLOSING_TRIGGERS = [
    ...
    (re.compile(r"\b(?:i'?m\s+|i\s+am\s+|have\s+to\s+|had\s+to\s+|gonna\s+|gotta\s+|need\s+to\s+)?cutt?ing\b", re.IGNORECASE), "cutting"),
    (re.compile(r"\b(?:have|had|gonna|gotta|need)\s+to\s+cut\b", re.IGNORECASE), "cutting"),
    (re.compile(r"\bcut\s+(?:these|this|my|the|it)\b", re.IGNORECASE), "cut"),
